<template>
	...
	<v-row >
	   <v-col cols="6">
      <v-card class="mx-auto" @click="setWindow(1)">
        <v-card-title>진행중</v-card-title>

        <v-dialog v-model="viewWork">
        <template v-slot:activator="{ on:dial }">
        <div>
          <div v-for="(works, index) in jobs" v-if="works.to!=undefined" class="cards">
            <div v-if="works.flag_expired == false">
                <v-row v-on="dial"  @click="setWork(works, index)">
                
                <div class="cards-jobs" v-if="works.to != undefined">
                  <v-chip
                    class="ma-2"
                    color="primary"
                  ...
                  <v-icon v-if="works.to.length>3">more_horiz</v-icon>
                </div>
      ...
      <v-list 
          ...
          <div v-for="(works, index) in jobs" class="cards">
